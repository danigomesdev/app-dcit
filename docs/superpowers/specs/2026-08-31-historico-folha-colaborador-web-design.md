...
- `groupByDay` (`/folha`) pareia por índice sobre todo o histórico (mesma convenção pré-existente do resto do sistema, sem discriminador in/out no `TimeEntry`). Um único punch-out esquecido desalinha todos os pares seguintes, podendo render uma duração implausível (ex.: "24h 00min") que o colaborador pode exportar em PDF. Fica registrado como pendência: corrigir exige mudança de modelo de dados ou um fluxo de ajuste — mitigação mais barata sugerida (não implementada aqui) é sinalizar pares acima de ~16h como "dia inconsistente" em vez de imprimi-los como horas trabalhadas.
